...
Atomic resolution could not be achieved during the experiment. A set of bibliographical images taken by other groups in similar conditions with the same equipment were analyzed instead (Figure~\ref{fig:atomic_resolution}). For better precision the images extracted were processed using cross correlation in the software WSxM.
...
The measured lattice constant $a$ corresponds to $a=\sqrt{3}a_{c-c}$, where $a_{c-c}$ is the nearest-neighbor distance of HOPG. The literature results of those measurements correspond to $a=2.46$\AA~and $a_{c-c}=1.42$\AA.

Our results measured an average value of $a=2,30$\AA~in the \ang{120} direction, and $a=2,44$\AA~in the \ang{60} direction.
