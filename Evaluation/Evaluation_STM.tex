...
The atomic-resolution images from the previous task are then analyzed using WSxM's line profile and distance measurement tools in the Representation menu of the software.
The values of the lattice constant $a$ was determined from the average distance between lattice unit cells for both directions (\ang{60} and \ang{120}), extracted from the Z bright intensity peaks (apparent corrugation height) over the distance using the software; these values are then compared to the known nearest-neighbor distance of \SI{1.42}{\angstrom}.

The $x/y$ calibration factors of the piezo scanner are, if necessary, adjusted to account for any measurement distortion. The angular relations between lattice directions are then evaluated to identify possible distortions due to drift or scanner nonlinearity.
