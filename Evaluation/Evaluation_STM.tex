...
\begin{figure}[H]
    ...
    \caption{
        ...
        The primitive vectors $\vec a_1$ and $\vec a_2$ connect neighbouring hexagon centres and span the triangular Bravais lattice.
...
\]
is the lattice constant of the triangular Bravais lattice, expressed in terms of the nearest–neighbour carbon–carbon distance $a_{\mathrm{CC}} = 1.42~\text{\AA}$.
...
\subsubsection{Reciprocal Lattice, Fourier Transforms, and Interpretation of STM Data}

The periodicity of the graphene lattice also manifests itself in \emph{reciprocal space}.
Any two–dimensional periodic pattern can be decomposed into plane waves, and the wavevectors that appear correspond to the reciprocal lattice vectors of the underlying Bravais lattice.
The Fourier transform (FFT) of an STM topograph therefore contains sharp peaks at the reciprocal lattice vectors of the triangular lattice spanned by $\vec a_1$ and $\vec a_2$.

The reciprocal lattice vectors are defined by
\[
\vec a_i \cdot \vec b_j = 2\pi\,\delta_{ij},
\]
which yields for graphene
\[
|\vec b_1| = |\vec b_2| = \frac{4\pi}{3\,a_{\mathrm{CC}}}.
\]
Their directions are rotated by $60^\circ$ relative to one another, forming a hexagonal reciprocal lattice.
The six first–order reciprocal lattice points appear as six bright spots arranged in a regular hexagon in the FFT of an atomic–resolution STM image.

This is an important conceptual point:
although only every second atom appears bright in real space because of AB stacking, the \emph{periodicity} is still that of the triangular Bravais lattice.
The missing sublattice modifies the \emph{intensities} of the Fourier peaks but not their \emph{positions}.
Thus the FFT still shows a perfect hexagon whose radius is determined solely by the lattice constant.

Because the magnitude of the reciprocal lattice vector is known analytically, the FFT allows a convenient determination of the carbon–carbon bond length.
If the first–order peak in the FFT appears at a wavevector magnitude $|\vec b|$, the corresponding bond length follows from
\[
a_{\mathrm{CC}} = \frac{4\pi}{3|\vec b|},
\qquad
a = \sqrt{3}\,a_{\mathrm{CC}}
\]
for the graphene lattice constant $a = |\vec a_1| = |\vec a_2|$.
In this way the reciprocal–space analysis complements the real–space imaging and provides a quantitative tool for characterizing the graphite surface.

In summary, the honeycomb lattice of graphene and the sublattice asymmetry of AB–stacked graphite determine the appearance of the STM images, while the triangular Bravais lattice determines the structure of their Fourier transforms.
Together these perspectives explain both the real–space contrast and the reciprocal–space features that will be analysed in the evaluation section.
